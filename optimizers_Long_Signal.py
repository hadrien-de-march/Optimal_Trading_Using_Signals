...
#        return np.around((self.M0-M)/self.sigma)
    ...
    def index_w(self,t,L):
        return max(0,min(t+L,2*self.Lsup))
...
#        return self.pPhasegrid_Alterna_q_t_Q[t,np.around((P-self.P0)/self.sigma+t),Q,np.around((self.M0-M)/self.sigma)]
    ...
    def Init_dics(self , mode, sigma , T , P0, M0 , Q0 , Qf , Gain , RiskAvers , Limpact , Lmax , Lsup , experiment_name=None):
        """
...
#        return
    ...
    def optim_Alterna_quantile_t_Q_L(self, mode_g, sigma, T, P0, M0, Q0, Qf, Gain , RiskAvers , Limpact , Lmax , Lsup , experiment_name=None ):
        """
        optimization

        :param mode: the optimization mode_g in 'basic', 'explicit', 'tabulated'
        
        Now only works for riskAvers = 0
        """
        ...
        if self.virgin:
            self.Init_dics(mode_g , sigma , T , P0, M0 , Q0 , Qf , Gain , RiskAvers , Limpact , Lmax , Lsup , experiment_name)
        ...
        #La matrice des esperances de U avec les termes a partir de t
        self.EU    = np.zeros((T+1,T+1,2*self.Lsup+1))
        
        #La matrice des esperances de Perf. Elle sera utile
        
        self.bkwEPerf = np.zeros((T+1,T+1,2*self.Lsup+1))

        #La matrice des Quantiles optimaux
        self.quant = np.zeros((T+1,T+1,2*self.Lsup+1))
        self.quant = self.quant-1

        # initialisation de l'algo backwards
        print("Initial Utility computing")
        for Q in range(T+1):
            for w in range(min(T+1,2*self.Lsup+1)):
                   self.EU[T,Q,w]=Utility(Penalization(self.P0,Q))
                   self.quant[T,Q,w] = 0
                   self.bkwEPerf[T,Q,w] = Penalization(self.P0,Q)/self.M0
        ...
        for j in range(T):
            ...
            for Q in range(t+1):
                for w in range(min(t+1,2*self.Lsup+1)):
                   L = w-t
                   sg = 2*(L>=0)-1
                   pL = np.abs(L)*1./self.Lmax
                #yes or no : the question is : "is there an action of the signal ?"
                #plus or moins : "What is the evolution of the signal ?"
                   w = lambda L0 : self.index_w(t+1,L0) 
                   
                   lambdaSupNoPlus  = self.EU[t+1,Q+1,w(L+1)]
                   lambdaSupYesPlus = self.EU[t+1,Q+1,w(L+1-sg)]
                   lambdaInfNoPlus  = self.EU[t+1,Q,w(L+1)]
                   lambdaInfYesPlus = self.EU[t+1,Q,w(L+1-sg)]

                   lambdaSupNoMoins  = self.EU[t+1,Q+1,w(L-1)]
                   lambdaSupYesMoins = self.EU[t+1,Q+1,w(L-1-sg)]
                   lambdaInfYesMoins = self.EU[t+1,Q,w(L-1-sg)]
                   lambdaInfNoMoins  = self.EU[t+1,Q,w(L-1)]


                   if mode_g == 'basic': 
                        q = 0.
                        qOpt = 1.
                        EvolOpt = pL/2*(lambdaSupYesPlus+lambdaSupYesMoins) + (1-pL)/2*(lambdaSupNoPlus+lambdaSupNoMoins) + self.Limpact*self.sigma*((self.Qf-Q)*pL-pL-(self.Qf-Q-1))/self.M0
            #L'algorithme "naif" d'optimisation
                        for i in range(self.Nmax):
                           PSupYes = pL*q
                           PInfYes = pL*(1-q)
            ...
            #On ajoute les morceaux du candidat au fur et a mesure
                           Candidat = (1.-self.gImpact(Q))*self.g(q)/self.M0
                           Candidat += -q*self.RiskAvers/2.*self.sigma**2*(2.*(Q-self.Qf)+1.)/M0**2
                           Candidat += -q*(self.Qf-Q-1.)*self.sigma*self.mImpact(Q)/self.M0
                           Candidat += self.Limpact*self.sigma*(q*pL-pL*(self.Qf-Q))/self.M0
                           Candidat += PSupYes/2*(lambdaSupYesPlus+lambdaSupYesMoins) + PInfYes/2*(lambdaInfYesPlus+lambdaInfYesMoins)
                           Candidat += PInfNo/2*(lambdaInfNoPlus+lambdaInfNoMoins) + PSupNo/2*(lambdaSupNoPlus+lambdaSupNoMoins)
                     ...
                     a=(lambdaSupYesPlus+lambdaSupYesMoins -lambdaInfYesPlus-lambdaInfYesMoins)*pL/2
                     a+=(lambdaSupNoPlus+lambdaSupNoMoins -lambdaInfNoPlus-lambdaInfNoMoins)*(1-pL)/2
                     a+= self.Limpact*sg*self.sigma*pL/self.M0
                     a+= -self.RiskAvers/2.*self.sigma**2*(2.*(Q-self.Qf)+1.)/self.M0**2  -(self.Qf-Q-1.)*self.sigma*self.mImpact(Q)/self.M0 
...
#                     print('qOpt', qOpt)
                     ...
                     EvolOpt += -qOpt*self.RiskAvers/2.*self.sigma**2*(2.*(Q-self.Qf)+1.)/M0**2
                     EvolOpt += -qOpt*(self.Qf-Q-1.)*self.sigma*self.mImpact(Q)/self.M0
                     EvolOpt += qOpt*self.Limpact*sg*self.sigma*pL/self.M0
                     EvolOpt += -self.Limpact*sg*self.sigma*pL*(self.Qf-Q)/self.M0
                     EvolOpt += PSupYes/2*(lambdaSupYesPlus+lambdaSupYesMoins) + PInfYes/2*(lambdaInfYesPlus+lambdaInfYesMoins)
                     EvolOpt += PInfNo/2*(lambdaInfNoPlus+lambdaInfNoMoins) + PSupNo/2*(lambdaSupNoPlus+lambdaSupNoMoins)
                     
                   
                   dPerf = -qOpt*(self.Qf-Q-1.)*self.sigma*self.mImpact(Q)/self.M0
                   dPerf += (1.-self.gImpact(Q))*self.g(qOpt)/self.M0
                   dPerf += qOpt*self.Limpact*sg*self.sigma*pL/self.M0
                   dPerf += -self.Limpact*sg*self.sigma*pL*(self.Qf-Q)/self.M0
                   ...
                   w = lambda L0 : self.index_w(t,L0)
                   self.EU[t,Q,self.index_w(t,L)]=EvolOpt
                   self.quant[t,Q,self.index_w(t,L)]=qOpt
                   print('t',t,'Q',Q,'L',L,'w',w(L),'qOpt',qOpt)
                   
                   lambdaSupNoPlus  = self.bkwEPerf[t+1,Q+1,w(L+1)]
                   lambdaSupYesPlus = self.bkwEPerf[t+1,Q+1,w(L+1-sg)]
                   lambdaInfNoPlus  = self.bkwEPerf[t+1,Q,w(L+1)]
                   lambdaInfYesPlus = self.bkwEPerf[t+1,Q,w(L+1-sg)]

                   lambdaSupNoMoins  = self.bkwEPerf[t+1,Q+1,w(L-1)]
                   lambdaSupYesMoins = self.bkwEPerf[t+1,Q+1,w(L-1-sg)]
                   lambdaInfYesMoins = self.bkwEPerf[t+1,Q,w(L-1-sg)]
                   lambdaInfNoMoins  = self.bkwEPerf[t+1,Q,w(L-1)]
                       
                   self.bkwEPerf[t,Q,self.index_w(t,L)]=PSupYes/2*(lambdaSupYesPlus+lambdaSupYesMoins) + PInfYes/2*(lambdaInfYesPlus+lambdaInfYesMoins)
                   self.bkwEPerf[t,Q,self.index_w(t,L)] += PInfNo/2*(lambdaInfNoPlus+lambdaInfNoMoins) + PSupNo/2*(lambdaSupNoPlus+lambdaSupNoMoins)
                   self.bkwEPerf[t,Q,self.index_w(t,L)] += dPerf


        self.Opti = self.EU[0,self.Q0,0]
        ...
        self.LatestOpt = 'optim_Alterna_quantile'
    ...
    def Alterna_quantile_t_Q_L_controlled_trajectories(self, NbSimul, plot=True, fignum=1):
        """
        Generate random trajectories and controls them for the alternative quantile
        """
        import numpy as np
        Simul = np.zeros((NbSimul,self.T+1,6))
        ...
        for i in range(NbSimul):
            ...
            for j in range(self.T):
                ...
                if a<0:
                   print(a)
                   print(self.EU[self.T,:])
                   raise RuntimeError("The final penalization is too much")
                if L*sg>self.Lsup:
                   print(L)
                   raise RuntimeError("Lsup is not enough")
                   
                Simul[i,t-1,5] = CurrentPerf + self.bkwEPerf[t-1,Q,w(L)]
                Simul[i,t-1,0]= self.quant[t-1,Q,w(L)]
                print('t',t,'L',L,'w',w(L),'Q',Q,'q',self.quant[t-1,Q,w(L)])
                q=Simul[i,t-1,0]
                pL = np.abs(L)*1./self.Lmax
                ...
                impact = np.random.binomial(1,pL)
                VolL = 2*np.random.binomial(1,0.5)-1.

                PSupPlus = (1+self.mImpact(Q)+self.Limpact*impact*sg+(1.-self.gImpact(Q))*self.G(q)/self.sigma)/2.
                
                if q==1.:
                    PInfPlus = 0.
                else:
                    PInfPlus = (1.+self.Limpact*sg*impact-(1.-self.gImpact(Q))*self.G(q)*q/self.sigma/(1.-q))/2.
#                print("q",q)
...
#                print("PInfPlus",PInfPlus,"i",i,"q",q,"t",t,"P",P,"Q",Q,"M",M)
            ...
            M=Simul[i,self.T,3]
            L=Simul[i,self.T,4]
            Simul[i,self.T,5] = self.PnL(self.Qf,self.M0)(P,Q,M)/self.M0
...
#
#        #Vector field
#        X,Y = np.meshgrid( np.linspace(0,self.T,self.T+1),np.linspace(0,self.T,self.T+1) )
#        U = 1.
#        V=np.zeros((self.T+1,self.T+1))
...
#        plt.show()
    ...
    def phase_filling_t_Q_L(self):
        import numpy as np
        T=self.T
        print("Fast phase filling")
        self.pPhase = np.zeros((T+1,T+1,self.Lsup+1))
        self.pPhase[0,0,0] = 1.
        for t in range(T):
            for Q in range(t+1):
                for L in range(self.Lsup+1):
                  for i in range(2):
                    if self.pPhase[t,Q,L,i]!=0:
                        CurrentProb = self.pPhase[t,Q,L,i]
                        q=self.quant[t,Q,L,i]
                        if q==-1:
                            raise RuntimeError("Inaccessible state")
                        pL = L*1./self.Lmax
                        self.pPhase[t+1,Q,max(0,L-1),i] += CurrentProb*(1.-q)*pL/2
                        self.pPhase[t+1,Q+1,L,i] += CurrentProb*q*pL/2
                        self.pPhase[t+1,Q,L,i] += CurrentProb*(1.-q)*(1-pL)/2
                        self.pPhase[t+1,Q+1,min(self.Lsup,L+1),i] += CurrentProb*q*(1-pL)/2
        
        return
        
        
    def Perf_StandardDev_t_Q_L(self):
        """
        ...
        for t in range(T):
            ...
            for Q in range(t+1):
                for L in range(self.Lsup+1):
                    Prob = self.pPhase[t,Q,L]
                    q = self.quant[t,Q,L]
                    pL = L*1./self.Lmax
                    dPerf_t += Prob*(1.-self.gImpact(Q))*self.g(q)
                    dPerf_t += -Prob*(self.Qf-Q-1.)*q*self.sigma*self.mImpact(Q)
                    dPerf_t += Prob*q*self.Limpact*self.sigma*(-pL-(self.Qf-Q-1))
                    dPerf_t += Prob*self.Limpact*self.sigma*pL*(self.Qf-Q)
...
#            EcarType = np.sqrt(np.sum(Carr_t[T-1,:])-Perf**2)
        ...
        self.Penal = np.zeros((T+1,self.Lsup+1))
        print("Fast final penalization")
        for Q in range(T+1):
            for L in range(self.Lsup+1):
                   self.Penal[Q]=Penalization(self.P0,Q)
                   
        Moy = self.Penal*self.pPhase[T,:,:]
        Carr = (self.Penal)**2*self.pPhase[T,:,:]
        ...
        if self.LatestOpt == 'optim_Alterna_quantile':

            PerfWord = 'Perf_Optim'
            EcarTypeWord = 'EcarType_Optim'
...
#        self.dic_output_params['Comp_EcarType_VS_Alterna_q_t_Q']= (self.dic_output_params['EcarType_Alterna_q_t_Q']-self.dic_output_params['EcarType'])/ self.dic_output_params['EcarType']
    ...
    def evolutionQ_t_Q_L(self,Ext = 0): 
        """
        Ext is here if the final penalization is weak enough
        """
        ...
        self.evolQ = np.zeros((T+1,self.Qf+1+Ext))
        
        for t in range(T+1):
            for Q in range(min(t+1,self.Qf+1+Ext)):
                self.evolQ[t,Q] = np.sum(self.pPhase[t,Q,:])
        
        
        self.df_evolQDump = pd.DataFrame(self.evolQ,index=range(T+1),columns=range(self.Qf+1+Ext) ) 


    def plot_evolQ_t_Q_L(self, k=0, Qf=None, T=None, fignum=1, ylim=None, fz=20 , ):
        """
        ...
        fig = plt.figure( fignum)
        plt.clf()
        plt.contourf(self.df_evolQDump.index,self.df_evolQDump.columns,self.df_evolQDump.values.T,levels = np.linspace(0,1,15),
                     cmap=pylab.get_cmap('YlOrRd'))
        plt.xlabel('Time',fontsize=fz)
        plt.ylabel('Qt',fontsize=fz)
        plt.title('Density of Bought Quantity (Qt)',fontsize=1.5*fz)
