...
#           raise RuntimeError("Potentially negative price")
        ...
        self.dic_input_params = {'T': T, 'P0': P0, 'Qf': Qf, 'M0' : M0, 'sigma': sigma, 'GainCoeff': Gain,
                                 ...
                                 'L Market Impact': Limpact,'Lmax': self.Lmax}
...
#        return
    ...
    def optim_Alterna_quantile_t_Q_L(self, mode_g, sigma, T, P0, M0, Q0, Qf, Gain , RiskAvers , Limpact , Lmax , Lsup , experiment_name=None ):
        ...
        #La matrice des esperances de U avec les termes a partir de t
        self.EU    = np.zeros((T+1,T+1,self.Lsup+1))
        
        #La matrice des esperances de Perf. Elle sera utile
        
        self.bkwEPerf = np.zeros((T+1,T+1,self.Lsup+1))

        #La matrice des Quantiles optimaux
        self.quant = np.zeros((T+1,T+1,self.Lsup+1))
        self.quant = self.quant-1

        # initialisation de l'algo backwards
        print("Initial Utility computing")
        for Q in range(T+1):
            for L in range(min(T+1,self.Lsup+1)):
                   self.EU[T,Q,L]=Utility(Penalization(self.P0,Q))
                   self.quant[T,Q,L] = 0
                   self.bkwEPerf[T,Q,L] = Penalization(self.P0,Q)/self.M0
        ...
        for j in range(T):
            ...
            for Q in range(t+1):
                for L in range(min(t+1,self.Lsup+1)):
                #yes or no : the question is : "is there a decay ?"
                   lambdaSupNo  = self.EU[t+1,Q+1,min(self.Lsup,L+1)]
                   lambdaSupYes = self.EU[t+1,Q+1,L]
                   lambdaInfNo  = self.EU[t+1,Q,L]
                   lambdaInfYes = self.EU[t+1,Q,max(0,L-1)]
                   ...
                   if mode_g == 'basic': 
                        q = 0.
                        qOpt = 1.
                        EvolOpt = pL*lambdaSupYes + (1-pL)*lambdaSupNo + self.Limpact*self.sigma*((self.Qf-Q)*pL-pL-(self.Qf-Q-1))/self.M0
            #L'algorithme "naif" d'optimisation
                        for i in range(self.Nmax):
                           PSupYes = pL*q
                           PInfYes = pL*(1-q)
            ...
            #On ajoute les morceaux du candidat au fur et a mesure
                           ...
                           Candidat += self.Limpact*self.sigma*(q*(-pL-(self.Qf-Q-1))+pL*(self.Qf-Q))/self.M0
                           Candidat += PSupYes*lambdaSupYes + PInfYes*lambdaInfYes + PInfNo*lambdaInfNo + PSupNo*lambdaSupNo
                     ...
                     a=(lambdaSupYes -lambdaInfYes)*pL + (lambdaSupNo -lambdaInfNo)*(1-pL) - self.Limpact*self.sigma*(pL+self.Qf-Q-1)/self.M0
...
#                     print('qOpt', qOpt)
                     ...
                     EvolOpt += qOpt*self.Limpact*self.sigma*(-pL-(self.Qf-Q-1))/self.M0
                     EvolOpt += self.Limpact*self.sigma*pL*(self.Qf-Q)/self.M0
                     EvolOpt += PSupYes*lambdaSupYes + PInfYes*lambdaInfYes + PInfNo*lambdaInfNo + PSupNo*lambdaSupNo
                     
                   
                   dPerf = -qOpt*(self.Qf-Q-1.)*self.sigma*self.mImpact(Q)/self.M0
                   dPerf += (1.-self.gImpact(Q))*self.g(qOpt)/self.M0
                   dPerf += qOpt*self.Limpact*self.sigma*(-pL-(self.Qf-Q-1))/self.M0
                   dPerf += self.Limpact*self.sigma*pL*(self.Qf-Q)/self.M0
                   ...
                   self.EU[t,Q,L]=EvolOpt
                   self.quant[t,Q,L]=qOpt
                   self.bkwEPerf[t,Q,L]=PSupYes*self.bkwEPerf[t+1,Q+1,L] + PSupNo*self.bkwEPerf[t+1,Q+1,min(self.Lsup,L+1)]
                   self.bkwEPerf[t,Q,L]+= PInfYes*self.bkwEPerf[t+1,Q,max(0,L-1)] + PInfNo*self.bkwEPerf[t+1,Q,L] + dPerf
    ...
    def Alterna_quantile_t_Q_L_controlled_trajectories(self, NbSimul, plot=True, fignum=1):
        ...
        for i in range(NbSimul):
            ...
            for j in range(self.T):
                ...
                Simul[i,t-1,5] = CurrentPerf + self.bkwEPerf[t-1,Q,L]
                Simul[i,t-1,0]= self.quant[t-1,Q,L]
                q=Simul[i,t-1,0]
                pL = L*1./self.Lmax
                ...
                PSupPlus = (1+self.mImpact(Q)+self.Limpact*(1-impact)+(1.-self.gImpact(Q))*self.G(q)/self.sigma)/2.
                
                if q==1.:
                    PInfPlus = 0.
                else:
                    PInfPlus = (1.-self.Limpact*impact-(1.-self.gImpact(Q))*self.G(q)*q/self.sigma/(1.-q))/2.
#                print("q", q)
        ...
        for t in range(T):
            for Q in range(t+1):
                for L in range(self.Lsup+1):
                    if self.pPhase[t,Q,L]!=0:
                        CurrentProb = self.pPhase[t,Q,L]
                        q=self.quant[t,Q,L]
                        if q==-1:
                            raise RuntimeError("Inaccessible state")
                        pL = L*1./self.Lmax
                        self.pPhase[t+1,Q,max(0,L-1)] += CurrentProb*(1.-q)*pL
                        self.pPhase[t+1,Q+1,L] += CurrentProb*q*pL
                        self.pPhase[t+1,Q,L] += CurrentProb*(1.-q)*(1-pL)
                        self.pPhase[t+1,Q+1,min(self.Lsup,L+1)] += CurrentProb*q*(1-pL)
